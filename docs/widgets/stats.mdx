...
<ParamField body="statsStyle" type="'default' | 'terminal' | 'minimal' | 'cards'" default="default">
  * `default`: Large 28px values with uppercase tracking labels below. * `terminal`: Monospace
  bracketed formatting (`[ 42 ]`). * `minimal`: Pure lightweight typography with no sublabels. *
...
<ParamField body="hideMetrics" type="string[]" default="[]">
  Array of metric IDs to hide. Available keys: `stars`, `repos`, `followers`, `following`, `forks`,
  `gists`.
</ParamField>
