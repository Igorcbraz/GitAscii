...
import type { RequestStarEmailPayload } from '@/lib/email/types'
...
export async function POST(request: Request) {
  try {
    ...
    const { users } = body as { users?: RequestStarEmailPayload[] }
    ...
    const results = await Promise.all(
      users.map(async (user) => {
        const result = await emailService.sendRequestStarEmail({
          email: user.email,
