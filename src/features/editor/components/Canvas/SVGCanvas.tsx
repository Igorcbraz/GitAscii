...
export function SVGCanvas() {
  ...
  const svgMarkupRef = useRef<HTMLDivElement>(null)
  ...
  const renderedSvgMarkup = useMemo(() => ({ __html: renderedSvgString }), [renderedSvgString])
  ...
  const applyPreviewToCanvas = useCallback((preview: DragPreview) => {
    ...
    if (!currentConfig || !svgMarkupRef.current) return
    ...
    for (const [instanceId, position] of Object.entries(preview.positions)) {
      ...
      const widgetElement = Array.from(
        svgMarkupRef.current.querySelectorAll<SVGGElement>('g[id]')
  ...
  useEffect(() => {
    const buildDragPreview = (event: MouseEvent) => {
      const activeDrag = activeDragRef.current
      const currentConfig = configRef.current
      if (!activeDrag || !currentConfig) return

      const deltaX = Math.round((event.clientX - activeDrag.startX) / zoomRef.current)
      const deltaY = Math.round((event.clientY - activeDrag.startY) / zoomRef.current)
    ...
    const finishDrag = () => {
      ...
      if (activeDrag && preview) {
        ...
        if (activeDrag.type === 'move') {
          ...
          if (positions.length > 0) store.updateWidgetPositions(positions)
        } else {
          ...
          if (
            ...
            store.updateWidgetSize(activeDrag.instanceId, size)
          }
        }
      }
