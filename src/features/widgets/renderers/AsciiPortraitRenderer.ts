...
export function renderAsciiPortrait(
  ...
  // Calculate grid rows based on default size
  const CELL_W = (width - PAD * 2) / COLS
  // We want to fit the rows into the remaining height
  const remainingHeight = height - TITLEBAR_H - STATUS_H - PAD

  // Use asciiText from config (generated by canvas on client-side) or generate default mockup
  let rowsTxt: string[] = []
  if (Array.isArray(cfg.asciiText) && cfg.asciiText.length > 0) {
    rowsTxt = cfg.asciiText
  } else {
    // Default rows count based on aspect ratio
  ...
  // Background
  parts.push(
    `<rect width="${width}" height="${height}" rx="${globalStyles.borderRadius || 12}" fill="url(#portrait-bg-${widget.instanceId})"/>`
  )
  parts.push(
    `<rect x="0.5" y="0.5" width="${width - 1}" height="${height - 1}" rx="${globalStyles.borderRadius || 12}" fill="none" stroke="${frameColor}" stroke-width="1"/>`
  )
  ...
  parts.push(
    `<text x="${width / 2}" y="${TITLEBAR_H / 2 + 4}" fill="${titleColor}" font-size="12" text-anchor="middle">${escapeXml(customTitle)}</text>`
  )
  ...
  for (let ry = 0; ry < ROWS; ry++) {
    ...
    // Blinking cursor riding the wipe edge
    parts.push(
  ...
  parts.push(
    `<line x1="0" y1="${statusLineY.toFixed(1)}" x2="${width}" y2="${statusLineY.toFixed(1)}" stroke="${frameColor}"/>`
  )
  parts.push(
