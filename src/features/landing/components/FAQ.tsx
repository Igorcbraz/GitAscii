...
export function FAQ() {
  ...
  return (
    <section id="faq" className="relative w-full bg-carbon py-24 px-4 sm:px-6 lg:px-8 border-t border-graphite">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-[1px] bg-signal-lime shadow-[0_0_10px_rgba(197,255,74,0.5)]"></div>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-6">
                    <span className="font-jetbrains-mono text-[13px] text-ash/70 group-hover:text-ash transition-colors">
                      {qNumber}
                    </span>
                    <span className={`font-inter-tight font-medium text-[16px] transition-colors duration-200 ${isOpen ? 'text-signal-lime' : 'text-chalk group-hover:text-chalk'}`}>
                      {faq.question}
                    </span>
                  </div>
                  <ChevronDown
