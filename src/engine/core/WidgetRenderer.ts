...
function renderExternalWidgetSvg(
  ...
  const externalBadgeSvg = `
    <g transform="translate(${Math.max(160, width - 150)}, 16)">
      <rect x="0" y="0" width="134" height="18" rx="3" fill="#121212" stroke="${accent}" stroke-width="0.8" opacity="0.9" />
      <circle cx="10" cy="9" r="2.5" fill="${accent}" />
      <text x="18" y="12" font-family="'JetBrains Mono', monospace" font-size="9" font-weight="600" fill="${accent}" letter-spacing="0.5">EXTERNAL WIDGET</text>
    </g>
  `;

  const imgStyle = mode === 'badge'
  ...
  const onerrorAttr = fallbackUrl ? ` onerror="this.onerror=null;this.src='${escapeXml(fallbackUrl)}';"` : '';
...
export function renderWidgetSvg(
  ...
  switch (widget.widgetId) {
    ...
    case 'ascii-art': {
      ...
      const asciiLines = asciiText || generateAsciiArt(data.user.login, {
    ...
    case 'bio': {
      const customBio = cfg.customBio !== undefined ? (cfg.customBio as string) : (data.user.bio || 'No bio provided.');
      const customLocation = cfg.customLocation !== undefined ? (cfg.customLocation as string) : (data.user.location || '');
      const customBlog = cfg.customBlog !== undefined ? (cfg.customBlog as string) : (data.user.blog || '');
    ...
    case 'footer': {
      contentSvg = `
        ...
        <text x="${width - 24}" y="${height / 2 + 4}" text-anchor="end" font-family="'JetBrains Mono', monospace" font-size="11" fill="${accent}">gitascii.com/${data.user.login}</text>
      `;
      break;
    }

    case 'tech-stack': {
      const selectedTechs = Array.isArray(cfg.selectedTechs) && cfg.selectedTechs.length > 0
    ...
    case 'social-media': {
      const selectedSocials = Array.isArray(cfg.selectedSocials) && cfg.selectedSocials.length > 0
      ...
      if (badgeStyle === 'skillicons') {
        ...
        const socialPlatformsMap: Record<string, { label: string; logo: string; color: string; defaultUrl: string }> = {
          github: { label: 'GitHub', logo: 'github', color: '181717', defaultUrl: 'https://github.com/{username}' },
          linkedin: { label: 'LinkedIn', logo: 'linkedin', color: '0A66C2', defaultUrl: 'https://linkedin.com/in/{username}' },
          twitter: { label: 'X', logo: 'x', color: '000000', defaultUrl: 'https://x.com/{username}' },
          discord: { label: 'Discord', logo: 'discord', color: '5865F2', defaultUrl: 'https://discord.gg/yourserver' },
          youtube: { label: 'YouTube', logo: 'youtube', color: 'FF0000', defaultUrl: 'https://youtube.com/@{username}' },
          instagram: { label: 'Instagram', logo: 'instagram', color: 'E4405F', defaultUrl: 'https://instagram.com/{username}' },
          twitch: { label: 'Twitch', logo: 'twitch', color: '9146FF', defaultUrl: 'https://twitch.tv/{username}' },
          devto: { label: 'Dev.to', logo: 'devto', color: '0A0A0A', defaultUrl: 'https://dev.to/{username}' },
          medium: { label: 'Medium', logo: 'medium', color: '000000', defaultUrl: 'https://medium.com/@{username}' },
          email: { label: 'Email', logo: 'gmail', color: 'EA4335', defaultUrl: 'mailto:user@example.com' },
          website: { label: 'Portfolio', logo: 'googlechrome', color: '2563EB', defaultUrl: 'https://{username}.dev' },
          stackoverflow: { label: 'StackOverflow', logo: 'stackoverflow', color: 'F48024', defaultUrl: 'https://stackoverflow.com/users/{username}' },
          bluesky: { label: 'Bluesky', logo: 'bluesky', color: '1185FE', defaultUrl: 'https://bsky.app/profile/{username}' },
          mastodon: { label: 'Mastodon', logo: 'mastodon', color: '6364FF', defaultUrl: 'https://mastodon.social/@{username}' },
          reddit: { label: 'Reddit', logo: 'reddit', color: 'FF4500', defaultUrl: 'https://reddit.com/user/{username}' },
          spotify: { label: 'Spotify', logo: 'spotify', color: '1DB954', defaultUrl: 'https://open.spotify.com/user/{username}' },
          telegram: { label: 'Telegram', logo: 'telegram', color: '26A5E4', defaultUrl: 'https://t.me/{username}' },
          tiktok: { label: 'TikTok', logo: 'tiktok', color: '000000', defaultUrl: 'https://tiktok.com/@{username}' },
          steam: { label: 'Steam', logo: 'steam', color: '000000', defaultUrl: 'https://steamcommunity.com/id/{username}' },
          hashnode: { label: 'Hashnode', logo: 'hashnode', color: '2962FF', defaultUrl: 'https://hashnode.com/@{username}' },
        };
        ...
        const badgesSvg = selectedSocials.map((platformId) => {
          const p = socialPlatformsMap[platformId];
          if (!p) return '';

          const label = p.label;
          const badgeW = badgeStyle === 'for-the-badge'
          ...
          const targetUrl = socialUrls[platformId] || p.defaultUrl.replace('{username}', data.user.login);
    ...
    case 'contribution-snake': {
      ...
      const snakeFileName = theme === 'light' ? 'github-contribution-grid-snake.svg' : 'github-contribution-grid-snake-dark.svg';
