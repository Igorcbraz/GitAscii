...
export function validateKeywordsCsv(text) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const [keyword, volume, kd, intent, priority, cluster, isPillar, aiOverviewPresent, source] =
      row
...
export function validatePlanCsv(text, references = {}) {
  ...
  if (!keywordSet)
    warnings.push('keywords.csv not provided; skipped keyword cross-reference validation')
  if (!promptSet)
    warnings.push('prompts.csv not provided; skipped prompt cross-reference validation')
...
function parseWithHeader(text, expectedHeader, errors) {
  ...
  if (
    ...
    header.some((value, i) => value !== expectedHeader[i])
  ) {
...
function buildPromptSet(text) {
  ...
  return new Set(
    rows
      .slice(1)
      .map((row) => normalizePromptRef(row[0]))
      .filter(Boolean)
  )
}
