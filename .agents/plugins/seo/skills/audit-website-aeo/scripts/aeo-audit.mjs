...
const UA =
  'Mozilla/5.0 (compatible; AEOAuditBot/1.0; +https://github.com/onvoyage-ai/gtm-engineer-skills)'
...
function shouldSkipPath(pathname) {
  return /\.(pdf|jpg|jpeg|png|gif|webp|svg|ico|zip|rar|7z|mp4|mp3|mov|avi|woff2?|ttf|eot|css|js)$/i.test(
    pathname
  )
}
...
export function extractAiView(html, pageUrl) {
  ...
  const h1 = allInner(html, 'h1')
    .map((t) => collapse(stripTags(t)))
    .filter(Boolean)
    .slice(0, 8)
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (item.datePublished)
          fields.push(`datePublished=${String(item.datePublished).slice(0, 20)}`)
        if (item.dateModified) fields.push(`dateModified=${String(item.dateModified).slice(0, 20)}`)
        if (item.publisher?.name)
          fields.push(`publisher=${String(item.publisher.name).slice(0, 60)}`)
  ...
  const publishedDate =
    metaByProp['article:published_time'] ||
    metaByName.date ||
    metaByName['dc.date'] ||
    timeDatetime ||
    ''
  const modifiedDate = metaByProp['article:modified_time'] || metaByName['last-modified'] || ''
...
export function scorePageChecks(v) {
  ...
  for (let i = 1; i < v.headingLevels.length; i += 1) {
    if (v.headingLevels[i] - v.headingLevels[i - 1] > 1) {
      headingSkipped = true
      break
    }
  }
  ...
  checks.push({
    ...
    passed: v.metaDescription.length >= 50,
    points: 10,
    details: v.metaDescription
      ? `Found description (${v.metaDescription.length} chars).`
      : 'No meta description found.',
  })
  checks.push({
    id: 'canonical',
    label: 'Canonical URL',
    passed: v.canonical.length > 0,
    points: 8,
  ...
  checks.push({
    ...
    passed: v.jsonLdCount > 0,
    points: 8,
    details:
      v.jsonLdCount > 0
        ? `Found ${v.jsonLdCount} JSON-LD block(s).`
        : 'No JSON-LD structured data found.',
  })
  checks.push({
    id: 'schema-types',
    label: 'Schema types identified',
    passed: foundTypes.length > 0,
    points: 8,
    details:
      foundTypes.length > 0
        ? `Types: ${foundTypes.join(', ')}`
        : 'No recognized @type found in JSON-LD.',
  })
  checks.push({
    id: 'og',
    ...
    points: 8,
    details: v.hasOg
      ? 'og:title and og:description found.'
      : 'Missing og:title and/or og:description.',
  })
  checks.push({
    id: 'internal-links',
    label: 'Internal linking',
    passed: v.internalLinkCount >= 5,
    points: 10,
  ...
  checks.push({
    ...
    details:
      v.imgCount === 0
        ? 'No images on page.'
        : `${v.imgsWithAlt}/${v.imgCount} images include alt text.`,
  })
  checks.push({
    id: 'text-depth',
    label: 'Readable content depth',
    passed: v.bodyWordCount >= 250,
    points: 12,
    details: `Estimated ${v.bodyWordCount} words in body text.`,
  })
  checks.push({
    id: 'indexability',
    label: 'Indexable for discovery',
    passed: !v.hasNoindex,
    points: 10,
  ...
  checks.push({
    id: 'ai-meta-tags',
    label: 'AI-accessible meta tags',
    passed: v.aiMetaTags.length === 0,
    points: 6,
    details:
      v.aiMetaTags.length === 0
        ? 'No restrictive AI meta tags.'
        : `Restrictive tags: ${v.aiMetaTags.join(', ')}`,
  })
  checks.push({
    id: 'heading-hierarchy',
    label: 'Content structure',
    passed: !headingSkipped && v.headingLevels.length > 0 && uniqueLevels >= 2,
    points: 6,
    details:
      v.headingLevels.length === 0
  ...
  async function processUrl(currentUrl, isFirst) {
    try {
      ...
      pages.push({
        ...
        checks,
        aiView,
        pageType: classifyPageType(currentUrl),
      })
...
export function buildCoverage(pageAudits) {
  ...
  return [...map.entries()]
    .map(([pageType, row]) => ({
      pageType,
      pages: row.pages,
      avgScore: Math.round(row.scoreSum / row.pages),
    }))
...
export function buildPrioritizedFixes(checks, coverage, llmsValid, blockedBots) {
  ...
  if (failed('ai-meta-tags'))
    fixes.push({
      title: 'Remove restrictive AI meta tags',
      impact: 'High',
      effort: 'Low',
  ...
  if (failed('schema'))
    fixes.push({
      title: 'Add structured data (JSON-LD) on key templates',
      impact: 'High',
      effort: 'Medium',
      why: 'Agents rely on machine-readable entities and page intent signals.',
    })
  if (failed('meta-description') || failed('title'))
    fixes.push({
      title: 'Standardize title and meta description quality',
      impact: 'High',
      effort: 'Low',
      why: 'Weak metadata reduces retrieval accuracy and snippet quality.',
    })
  if (failed('text-depth'))
    fixes.push({
      title: 'Expand answer-first content on thin pages',
      impact: 'High',
      effort: 'Medium',
      why: 'Agents need enough explicit facts and context to answer safely.',
    })
  if (failed('internal-links'))
    fixes.push({
  ...
  if (!llmsValid)
    fixes.push({
      title: 'Publish a valid llms.txt with heading, links, and policy info',
      impact: 'Medium',
      effort: 'Low',
  ...
  if (failed('rss-feed'))
    fixes.push({
      title: 'Add an RSS or Atom feed',
      impact: 'Medium',
      effort: 'Low',
      why: 'RSS feeds help AI systems discover and track new content.',
    })
  if (failed('heading-hierarchy'))
    fixes.push({
      title: 'Improve content structure with clear heading levels',
      impact: 'Medium',
      effort: 'Low',
      why: 'Well-structured H1 → H2 → H3 outlines help AI agents chunk, extract, and cite specific sections.',
    })
  if (failed('schema-types'))
    fixes.push({
      title: 'Add recognized schema types to JSON-LD',
      impact: 'Medium',
      effort: 'Medium',
      why: 'Named types like Organization, Product, Article help AI classify your content.',
    })
  if (!coverage.some((c) => c.pageType === 'docs'))
    fixes.push({
      title: 'Create a docs/knowledge section for product facts',
      impact: 'Medium',
      effort: 'Medium',
...
function checkPassRatio(pageAudits, id) {
  ...
  return (
    pageAudits.filter((p) => p.checks.find((c) => c.id === id)?.passed).length / pageAudits.length
  )
}
const scoreStatus = (s) => (s >= 80 ? 'Strong' : s >= 60 ? 'Moderate' : 'Weak')
const priorityFromScore = (s) =>
  s < 45 ? 'Critical' : s < 65 ? 'High ROI' : s < 82 ? 'Quick Win' : 'Monitor'
...
export function buildIntelligenceSignals(ctx) {
  ...
  const numericEvidenceRatio =
    pageAudits.filter((p) => /\d/.test(p.aiView.textExcerpt)).length / total
  const headingRichRatio = pageAudits.filter((p) => p.aiView.headings.length >= 4).length / total
  const faqHeadingRatio =
    pageAudits.filter((p) => p.aiView.headings.some((h) => /\?|FAQ|how to|what is|guide/i.test(h)))
      .length / total
  const answerFirstRatio =
    pageAudits.filter((p) =>
      /\bis\b|\bare\b|\bwas\b|\bmeans\b|\brefers to\b|\bdefined as\b/i.test(
        (p.aiView.textExcerpt || '').slice(0, 200)
      )
    ).length / total
  const hasDateSignals =
    pageAudits.filter((p) => p.aiView.publishedDate || p.aiView.modifiedDate).length / total
  const hasAuthorRatio = pageAudits.filter((p) => !!p.aiView.author).length / total

  const base = [
    {
      id: 'answer-readiness',
      signal: 'Answer Readiness',
      score: scoreToInt(
        30 * faqHeadingRatio + 30 * answerFirstRatio + 20 * metaRatio + 20 * textDepthRatio
      ),
    ...
    },
    {
      ...
      score: scoreToInt(
        35 * numericEvidenceRatio +
          25 * hasAuthorRatio +
          20 * textDepthRatio +
          20 * internalLinksNorm
      ),
    ...
    },
    {
      ...
      score: scoreToInt(
        35 * textDepthRatio +
          25 * templateDiversity +
          25 * internalLinksNorm +
          15 * headingRichRatio
      ),
    ...
    },
    {
      ...
      score: scoreToInt(
        30 * headingHierarchyRatio + 25 * titleRatio + 25 * singleH1Ratio + 20 * headingRichRatio
      ),
...
`)
  ...
  const report = {
    ...
    scoring: {
      ...
      note:
        'provisionalScore uses a deterministic heuristic for the intelligence half. ' +
...
function printSummary(r) {
  ...
  console.log(
    `  Foundational score:      ${r.scoring.foundationalScore}/100  (16 deterministic checks)`
  )
  console.log(
    `  Heuristic intelligence:  ${r.scoring.heuristicIntelligenceScore}/100  (deterministic prior)`
  )
  console.log(
    `  Provisional score:       ${r.scoring.provisionalScore}/100  → grade ${r.scoring.provisionalGrade}`
  )
