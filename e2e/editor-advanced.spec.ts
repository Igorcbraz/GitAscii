...
test.describe('GitAscii Advanced Editor E2E Tests', () => {
  ...
  test.skip('LayersPanel: selection and reordering', async ({ page }) => {
    ...
    const widgetsInCanvas = page.locator('[data-testid="canvas-svg-container"] svg > g[data-widget-id]')
  ...
  test('Multiple selection: should allow selecting multiple widgets', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="canvas-widget-bio"]')).toHaveAttribute('data-selected', 'true')
    await expect(page.locator('[data-testid="canvas-widget-github-stats"]')).toHaveAttribute('data-selected', 'true')
  })
  ...
  test('Widget resizing: should allow resizing widgets', async ({ page }) => {
    ...
    if (await resizeHandle.isVisible()) {
      ...
      await expect(page.locator('[data-testid="canvas-widget-bio"]')).not.toHaveAttribute('data-width', initialWidth)
    }
  })
...
test.describe('GitAscii API Error and Invalid Cases', () => {
  ...
  test('Non-existent user', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/not found|404/i, { timeout: 3000 }).catch(() => {
